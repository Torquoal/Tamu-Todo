//
//  SleepScene.swift
//  TodoList
//
//  Created by Shaun Macdonald on 12/01/2018.
//  Copyright © 2018 Shaun Macdonald. All rights reserved.
//
//  See JumpScene for comment guide on the implementation of this and other SkScene classes
//

import SpriteKit

class SleepScene: SKScene {
    
    var Tamu = SKSpriteNode()
    
    var TextureAtlas = SKTextureAtlas()
    var TextureArray = [SKTexture]()
    
    override func didMove(to view: SKView) {
        
         let sleepSound = SKAction.playSoundFileNamed("Sounds/sleepTone.wav", waitForCompletion: false)
        
        TextureAtlas = SKTextureAtlas(named: "TamuSprites")
        
        let Bg = SKSpriteNode(imageNamed: "background")
        
        Bg.size = CGSize(width: frame.width, height: frame.height)
        
        Bg.position = CGPoint(x: frame.midX, y: frame.midY)
        
        self.addChild(Bg)
        
        for i in 0...26{
            
            let Name = "tamuSleep_\(i).png"
            TextureArray.append(SKTexture(imageNamed: Name))
        }
        
        Tamu = SKSpriteNode(imageNamed: TextureAtlas.textureNames[0] as String)
        
        Tamu.size = CGSize(width: frame.width/2.4, height: frame.height/4.1)
        
        Tamu.position = CGPoint(x: frame.width/1.34, y: frame.height/1.49)
        
        self.addChild(Tamu)
        
        // If sleep animarion takes place during sleeping hours add visual nightime filter
        let calendar = Calendar.current
        let now = Date()
        let bedtime = calendar.date(
            bySettingHour: 22,
            minute: 0,
            second: 0,
            of: now)!
        
        let morning = calendar.date(
            bySettingHour: 7,
            minute: 0,
            second: 0,
            of: now)!
        
        if (now <= morning || now >= bedtime){
            let filter = SKSpriteNode(imageNamed: "filter")
            
            filter.size = CGSize(width: frame.width, height: frame.height)
            
            filter.position = CGPoint(x: frame.midX, y: frame.midY)
            
            self.addChild(filter)
        }
        
        run(sleepSound)
        Tamu.run(SKAction.repeatForever(SKAction.animate(with: TextureArray, timePerFrame: 0.1)))
        
    }
}


